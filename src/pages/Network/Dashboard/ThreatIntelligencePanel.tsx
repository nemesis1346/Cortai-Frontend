...
const statusClassMap: Record<ThreatItem['status'], string> = {
	blocked: 'bg-emerald-500/20 text-emerald-300',
			...
			<CardHeader
				left={(
					<div className="flex items-center gap-2">
						<Search className="h-5 w-5 shrink-0 text-[#00D4C0]" strokeWidth={1.75} />
				...
				middle={(
					<div className="flex flex-wrap items-center justify-center gap-2">
						...
						<span className="rounded-[3px] bg-emerald-500/20 px-2 py-1 text-[11px] font-medium text-emerald-300">
							{s.risk}
						</span>
					</div>
				)}
				right={(
					<div className="flex items-center gap-0.5">
						<button type="button" className="rounded-md p-1.5 text-white/50 transition hover:bg-white/[0.06] hover:text-white" aria-label="Search threats">
							<Search className="h-4 w-4" strokeWidth={1.75} />
						</button>
						<button type="button" className="rounded-md p-1.5 text-white/50 transition hover:bg-white/[0.06] hover:text-white" aria-label="Filter threats">
			...
			<CardBody className="min-h-0 flex-1 overflow-auto pt-1">
				<div className="flex flex-row justify-between items-center py-4">
					<div className='flex flex-col items-start justify-start'>
						<div className="text-[18px] font-semibold leading-none tracking-tight text-white">{s.categories.malware}</div>
						<div className="mt-1 text-[11px] text-white/45">Malware</div>
					</div>
					<div className='flex flex-col items-start justify-start'>
						<div className="text-[18px] font-semibold leading-none tracking-tight text-white">{s.categories.phishing}</div>
						<div className="mt-1 text-[11px] text-white/45">Phishing</div>
					</div>
					<div className='flex flex-col items-start justify-start'>
						<div className="text-[18px] font-semibold leading-none tracking-tight text-white">{s.categories.intrusion}</div>
						<div className="mt-1 text-[11px] text-white/45">Intrusion</div>
					</div>
				</div>
				<div className="flex flex-col gap-2 mt-4">
					{items.map((item) => (
						<div
							key={item.id}
							className="grid grid-cols-[1.5fr_0.7fr_0.7fr_0.5fr] border-b border-white/[0.06] items-center gap-3 py-1 first:pt-0"
						>
							<div
								className={`min-w-0 text-[14px] font-medium leading-snug ${isTrojanType(item.type) ? 'text-red-400' : 'text-white/90'}`}
							>
								{item.type}
							</div>
							<div className="flex min-w-0 items-center gap-1.5 text-[14px] leading-snug text-white/45">
								<Cable className="h-3.5 w-3.5 shrink-0 text-[#00D4C0]" strokeWidth={2} aria-hidden />
								<span className="truncate">{item.source}</span>
							</div>
							<div className="shrink-0 whitespace-nowrap text-[14px] text-white/45">{item.time}</div>
