	...
	return (
		<Card className="rounded-2xl border border-border bg-card">
			<CardHeader
				...
				middle={<span />}
				right={<span className="text-[0.8125rem] text-text-dim">{s.today} today</span>}
			/>
				...
				<div className="grid grid-cols-1 gap-6 pt-4 sm:grid-cols-3 sm:gap-4 sm:pt-4">
					<div className="flex flex-col gap-4">
						<div className="flex items-center gap-2">
							...
							<span className="text-[1.125rem] font-bold leading-none text-danger">{s.slipFall}</span>
						</div>
						<div className="text-[0.75rem] text-text-dim">Slip/Fall</div>
					</div>
					<div className="flex flex-col gap-4">
						<div className="text-[1.125rem] font-bold leading-none text-warn">{s.equipMisuse}</div>
						<div className="text-[0.75rem] text-text-dim">Equip Misuse</div>
					</div>
					<div className="flex min-w-0 flex-col divide-y divide-border border-t border-border pt-4 sm:border-t-0 sm:pt-0 sm:pl-2">
						{s.log.map((row) => (
							<div key={row.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
								<span className="min-w-0 text-[0.8125rem] text-text-dim">{row.title}</span>
								<span className="inline-flex shrink-0 items-center gap-1.5 text-[0.8125rem] text-text-dim">
