					...
					<div key={z.label} className="flex flex-row items-center gap-2">
						<div className="flex flex-row items-stretch gap-1.5">
							<div className="relative w-3 shrink-0 flex items-end justify-center" style={{ height: 30 }}>
								<div
									className="absolute left-2 w-0 h-0 border-y-[5px] border-y-transparent border-l-[7px] border-r-[color:var(--primitive-white-shadow-100)]"
									style={{ bottom: `calc(${dbToBottomPercent(z.db)}% - 5px)` }}
								/>
							</div>
							<div className="flex h-[88px] w-2.5 shrink-0 flex-col rounded-sm overflow-hidden border border-[color:var(--primitive-white-shadow-20)]">
						...
						<div className="flex flex-col items-start gap-2">
							...
							<div className="text-[15px] text-text-dim text-center">{z.label}</div>
						</div>
					</div>
				))}
			</div>
