	...
	return (
		<div className="flex shrink-0 items-end gap-1">
			<div className="relative h-[52px] w-2.5 shrink-0">
				<div
					className="absolute -right-1 w-0 h-0 border-y-[5px] border-y-transparent border-l-[7px] border-r-[color:var(--primitive-white-shadow-100)]"
					style={{ bottom: `calc(${pct}% - 5px)` }}
				/>
			</div>
			<div className="h-[52px] w-2 shrink-0 overflow-hidden rounded-sm border border-[color:var(--primitive-white-shadow-20)]">
...
function StatBlock({ value, label }: { value: string; label: string }) {
	return (
		<div className="flex min-w-[3.25rem] flex-col items-center gap-1 text-center">
			<div className="text-[17px] font-semibold leading-tight text-text sm:text-[18px]">{value}</div>
	...
	return (
		<div className="flex shrink-0 items-center gap-2">
			...
			<div className="flex flex-col gap-2 justify-between">
				<span className="text-[19px] font-bold leading-none text-ok sm:text-[21px]">{tempLabel}</span>
				<span className="text-[11px] text-text-dim">Temp.</span>
			</div>
		</div>
	)
}
	...
	return (
		<div className="flex shrink-0 items-center gap-2">
			<Progress
				...
				format={() => null}
			/>
			<span className="text-[17px] font-medium text-text-dim">{percent}%</span>
		</div>
	)
}
		...
		<div className="rounded-2xl border border-border bg-panel p-4">
			<section className="pb-4">
				<div className="flex items-start justify-between gap-2">
					<div className="flex items-center gap-2 text-[18px] font-medium text-text">
						<Waves className="h-5 w-5 shrink-0 text-brand" />
						<span>Pool</span>
					</div>
					<span className="text-[12px] text-text-dim">{p.maxVisitors} max</span>
				</div>
			...
			<section className="border-t border-border py-4">
				<div className="flex items-start justify-between gap-2">
					...
					<span className="text-[12px] text-text-dim">{s.maxVisitors} max</span>
				</div>
				<div className="mt-4 flex flex-row items-center justify-between gap-x-3 gap-y-5 sm:gap-x-4 md:gap-x-6">
			...
			<section className="border-t flex flex-col gap-2 border-border pt-4 ">
				<div className="flex flex-wrap items-start justify-between gap-2">
					...
					<span className="text-[13px] text-text-dim">Total today: {g.totalToday}</span>
				</div>
				<div className="mt-4 flex flex-wrap items-start justify-between gap-4">
					<div className="min-w-[7rem]">
						<span className="text-[14px] text-brand">Adults</span>
						<span className="text-[14px] ml-2 text-text">{g.adultsNow} now</span>
						<span className="ml-2 text-[14px] text-text-dim">{g.adultsToday} today</span>
					</div>
					<span className="rounded-[3px] bg-panel px-2 py-1 text-[12px] text-text-dim">Ratio: {g.adults}:{g.kids}</span>
					<div className="min-w-[7rem] text-left sm:text-right">
						<span className="text-[13px] font-medium text-brand">Kids</span>
						<span className="text-[14px] ml-2 text-text">{g.kidsNow} now</span>
						<span className="ml-2 text-[13px] text-text-dim">{g.kidsToday} today</span>
					</div>
				</div>
				<div className="mt-4 h-[8px] overflow-hidden rounded-full bg-[color:var(--primitive-brand-900)]">
