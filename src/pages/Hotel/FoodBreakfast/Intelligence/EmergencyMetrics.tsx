			...
			<CardHeader
				left={
					<div className="text-white inline-flex items-center gap-2">
						<Clock className="w-5 h-5 text-[#00d4c0]" />
			...
			<CardBody className="flex flex-col gap-2">
				{['6:00','6:30','7:00','7:30','8:00','8:30','9:00','9:30'].map((t, idx) => (
					<div key={t} className="grid grid-cols-[28px_1fr_24px] items-center gap-2">
						<div className="text-[12px] text-white/60">{t}</div>
						<div className="h-2 rounded-full bg-white/10 overflow-hidden">
							<div className={`h-full rounded-full ${idx === 3 ? 'bg-rose-400' : 'bg-[#00d4c0]'} `} style={{ width: `${[20,38,72,100,85,64,48,22][idx]}%` }} />
						</div>
						<div className={`text-[12px] justify-self-end ${idx === 3 ? 'text-rose-400' : 'text-white/70'}`}>{[5,16,42,74,68,52,38,14][idx]}</div>
					</div>
				))}
			</CardBody>
		</Card>
