...
export default function RoomCell({
	...
	const hoverBg =
		...
		room.state === 'occupied' ? 'hover:bg-[color:var(--primitive-white-shadow-10)]' :
		...
		'hover:bg-[color:var(--primitive-white-shadow-10)]'
	return (
		<button
			type="button"
			onClick={onSelect}
			className={`w-full rounded-[10px] border px-3 py-2 text-left transition-colors cursor-grab active:cursor-grabbing ${active ? 'border-brand/60 bg-brand/5' : 'border-[color:var(--primitive-white-shadow-5)] bg-panel'}`}
		>
